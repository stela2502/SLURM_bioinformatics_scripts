...
## now let's add an external sbatch runner - NEEDS sbatch to acutually do anything!!
my $cmd = "RScript $plugin_path/data/sbatchRunner.R &";
#
#system( $cmd );

$obj-> AddDataset({'cmd' => "Not to be run", 'Finished' => 0 } );
$obj-> AddDataset({'cmd' => "sbatch simple.sh", 'Finished' => 0 } );
$obj-> AddDataset({'cmd' => "sbatch simple.sh", 'Finished' => 0 } );

is_deeply ( $obj->waiting(), 3, "3 waiting");

sleep(10);

is_deeply ( $obj->waiting(), 0, "0 waiting - processes have been cleared out");
